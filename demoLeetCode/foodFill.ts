...
export const floodFill = (
  ...
  fillColor(image, sr, sc, color, height, width, image[sr][sc]);
  return image;
};
...
const fillColor = (
  ...
  image[sr][sc] = color;
  fillColor(image, sr, sc + 1, color, height, width, originalColor);
  fillColor(image, sr, sc - 1, color, height, width, originalColor);
  fillColor(image, sr + 1, sc, color, height, width, originalColor);
  fillColor(image, sr - 1, sc, color, height, width, originalColor);
};
